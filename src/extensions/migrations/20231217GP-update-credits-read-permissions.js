...
async function getUserPermissions (policyId) {
	const URL = `${DIRECTUS_URL}/permissions?filter[collection][_eq]=credits&filter[policy][_eq]=${policyId}`;
	const response = await fetch(URL, {
		headers: {
			Authorization: `Bearer ${ADMIN_ACCESS_TOKEN}`,
		},
...
async function patchReadPermissions (readPermissions) {
	...
	const newFields = readPermissions.fields.filter(field => field !== 'githubLogin');
