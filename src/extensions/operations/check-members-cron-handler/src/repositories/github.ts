...
const githubGet = <T = unknown>(path: string, token: string, options: { maxRedirects?: number } = {}) => {
	return axios.get<T>(`${GITHUB_API_URL}${path}`, {
 ...
 */
export const findLeftMemberships = async (org: Org): Promise<string[]> => {
	...
	for (const member of membersWithToken) {
		const ownMembership = await getOwnMembership(org, member);
	...
	const verdicts = await getVerdicts(org, org.members, checker.githubOauthToken!);
	...
	for (const [ member, verdict ] of verdicts) {
		...
		if (verdict === 'unknown' && await hasLeftOrg(org, member, checker)) {
			left.push(member);
		}
	}
...
const getOwnMembership = async (org: Org, member: OrgMember): Promise<OwnMembership> => {
	const response = await githubGet<{ state: string }>(`/user/memberships/orgs/${org.name}`, member.githubOauthToken!);
...
const getVerdicts = async (org: Org, members: OrgMember[], token: string): Promise<Map<OrgMember, Verdict>> => {
	...
	for (let i = 0; i < membersWithLogin.length; i += USERS_BATCH_SIZE) {
		...
		const nodes = await fetchMembersByLogin(batch.map(member => member.githubUsername!), token);
...
const fetchMembersByLogin = async (logins: string[], token: string): Promise<GraphqlNode[]> => {
...
const hasLeftOrg = async (org: Org, member: OrgMember, checker: OrgMember): Promise<boolean> => {
	...
	const userResponse = await githubGet<{ login: string }>(`/user/${member.githubId}`, token);
	...
	const membershipResponse = await githubGet(`/orgs/${org.name}/members/${userResponse.data.login}`, token, {
