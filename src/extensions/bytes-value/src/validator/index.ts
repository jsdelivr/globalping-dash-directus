...
export default defineHook(({ filter }) => {
	...
	filter('gp_orgs.items.update', (payload) => {
		const org = payload as Partial<Org>;

		if (org.adoption_token !== undefined) {
			// hashBytes() validates that token was generated by the API.
			hashBytes(org.adoption_token);
			deleteBytes(org.adoption_token);
		}
	});
});
