...
const sendNotificationProbeAdopted = async (adoption: NotificationInfo, { services, getSchema }: EndpointExtensionContext) => {
	...
	await notificationsService.createOne({
		...
		message: `A new ${adoption.name ? `probe [${adoption.name}](/probes/${adoption.id})` : `[probe](/probes/${adoption.id})`} with IP address **${adoption.ip}** has been assigned to your account.`,
	});
};
